...
parse = parseAux 0 where
    ...
    splitOuterComma i (x:xs)
        ...
        | x == ']' = (x:xl2,xr2)
        | otherwise = (x:xl3,xr3)
            where (xl1,xr1) = splitOuterComma (i+1) xs 
                  (xl2,xr2) = splitOuterComma (i-1) xs
                  (xl3,xr3) = splitOuterComma i xs 
...
explode (Pair sn1 sn2 d)
    | isJust msg11 || isJust msg12 = (Pair sn1p (propagate sn2 msg12 LeftD) d, msg11, Nothing, True)
    | isExploded1 = (Pair sn1p sn2 d, Nothing, Nothing, True)
    | isJust msg21 || isJust msg22 = (Pair (propagate sn1 msg21 RightD) sn2p d,  Nothing, msg22, True)
    | otherwise = (Pair sn1 sn2p d, Nothing, Nothing, isExploded2)
        where (sn1p,msg11,msg12,isExploded1) = explode sn1
              ...
              propagate :: SnailNum -> Maybe Int -> Direction -> SnailNum
              propagate sn Nothing _ = sn
              propagate (Regular i d) (Just im) _ = Regular (i+im) d
              propagate (Pair sn1 sn2 d) msg RightD  = Pair sn1 (propagate sn2 msg RightD) d
              propagate (Pair sn1 sn2 d) msg LeftD = Pair (propagate sn1 msg LeftD) sn2 d
